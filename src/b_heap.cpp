...
int b_h_get_top( b_heap_t* b_h)
{
	...
	rtn= b_h->h_arr[1];
    //--
    //b_h->obj_arr[b_h->h_arr[1]].key=INFINITY;
    //--
...
/*
 *	modify from b_h_get_top and b_h_update_key
 *  obj_arr[k]
 */
void b_h_remove( b_heap_t* b_h,int k)
{
    
    int n_tmp, rtn;
	int pos= b_h->obj_arr[k].loc;
    /*
    printf("pos:%d\tb_h->h_arr[pos]:%d\n",pos,b_h->h_arr[pos]);
    printf("b_h->rear:%d\n",b_h->rear);
    printf("b_h:\n");
    print_b_heap(stdout, b_h);
    printf("\n");
*/
	...
	//Replace the top the rear element.
	b_h->h_arr[pos] = b_h->h_arr[b_h->rear];
	n_tmp = b_h->h_arr[pos];
	b_h->obj_arr[n_tmp].loc = pos;
	b_h->rear --;
  
	//Do the arrangement[top-to-buttom].
	//t_b_heapify( b_h->h_arr, pos, b_h->rear, b_h->obj_arr);
    b_h_update_key(b_h, b_h->h_arr[pos]);
  	return;
}
 ...
 *
 */
void b_h_restruct_heap( b_heap_t* b_h, int rear)
{
    free(b_h->h_arr);
    
    b_h->rear = 0;
    b_h->size = INI_HEAP_SIZE + 1;
    b_h->h_arr = ( int*)malloc( b_h->size * sizeof( int));
	memset( b_h->h_arr, 0, b_h->size * sizeof( int));

    for (int i =1; i<rear;i++){
//        printf("\ti:%d\n",i);
        if( b_h->obj_arr[i].key >=0){
//            printf("\t\ti:%d\n",i);
//            print_b_heap(stdout, b_h);
//            printf("\n");
            b_h_insert(b_h, i);
